...
function CountryMap(element, props) {
  ...
  function drawMap(mapData) {
    ...
    mapLayer
      ...
      .on('click', clicked)
      .each(function (d) {
        // Add metric text for each region
        const centroid = path.centroid(d);
        //const regionData = data.find(region => region.country_id === d.properties.ISO);
        const regionData = `${d.properties.ISO}\n${d.properties.NAME_1}`;
        const colors = ["red", "green", "blue"];
        //debugger;
        if (regionData && centroid[0] > 0 && centroid[1] > 0) {
          svg.append("text")
            ...
            .html(function () {
              return regionData.split("\n").map(function (d, i) {
                return `<tspan x="0" dy="${i * 1.2}em" fill="${colors[i]}">${d}</tspan>`;
              }).join("");
            })
        };
      });
